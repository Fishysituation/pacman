...
#define GHOST_FRAMES (MOVE_FRAMES * 1.1)
...
void InitGameplayScreen(int lifeCount) {
  //TODO change things to populate and add in one pass
  gameOver = false;
  pause = false;
  won = false;
  ...
  /* Image image = LoadImage("cherry.png");
  cherry = LoadTextureFromImage(image);
  UnloadImage(image); */
}
            ...
            for (int i = 0; i < NO_GHOSTS; i++) {
                switch (ghosts[i]->behavior) {
                    case CHASE:
                        ...
                        if (ghosts[i]->moveProgress == GHOST_FRAMES) {
                ...
                if (ghosts[i]->framesUntilActive > 0) {
                    ...
                    if (ghosts[i]->framesUntilActive == 0 && ghosts[i]->behavior != FLEE) {
                        ghosts[i]->behavior = CHASE;
                    }
                }                    
            }
...
//TODO: draw walls nicely?
//TODO: abstracat stuff like circle centring, make less magic
...
//TODO: change this to not be as awful
void DrawDoor() {
  for (int y = 0; y < HEIGHT; y++) {
...
void DrawConsumables() {
  for (int i = 0; i < NO_CONSUMABLES; i++) {
    if (consumables[i] != NULL && !consumables[i]->used) {
      ...
      if (consumables[i]->powerUp) {
        //DrawTexture(cherry, (x * SQUARE_SIZE) + (SQUARE_SIZE / 2) + X_OFFSET, (y * SQUARE_SIZE) + (SQUARE_SIZE / 2) + Y_OFFSET, BLACK);
...
//doesn't work with ghosts atm idk why
...
//TODO: change to use getCenter and drawGhost in animation.c
void DrawGhosts() {
  Color color1 = RED;
  Color color2 = PINK;
  ...
  if (ghosts[2]->behavior == FLEE) {
    ...
    Vector2 ghost1Center = (Vector2) {(ghost1.position->x * SQUARE_SIZE) + (SQUARE_SIZE / 2) + X_OFFSET - 
                      (Direction[ghost1.direction].x * SQUARE_SIZE * (1 - ((float) ghost1.moveProgress / GHOST_FRAMES))),
                      (ghost1.position->y * SQUARE_SIZE) + (SQUARE_SIZE / 2) + Y_OFFSET - 
                      (Direction[ghost1.direction].y * SQUARE_SIZE * (1 - ((float) ghost1.moveProgress / GHOST_FRAMES)))};
    ...
    Vector2 ghost2Center = (Vector2) {(ghost2.position->x * SQUARE_SIZE) + (SQUARE_SIZE / 2) + X_OFFSET - 
                      (Direction[ghost2.direction].x * SQUARE_SIZE * (1 - ((float) ghost2.moveProgress / GHOST_FRAMES))),
          (ghost2.position->y * SQUARE_SIZE) + (SQUARE_SIZE / 2) + Y_OFFSET - 
                      (Direction[ghost2.direction].y * SQUARE_SIZE * (1 - ((float) ghost2.moveProgress / GHOST_FRAMES)))};
    ...
    Vector2 ghost3Center = (Vector2){(ghost3.position->x * SQUARE_SIZE) + (SQUARE_SIZE / 2) + X_OFFSET - 
                      (Direction[ghost3.direction].x * SQUARE_SIZE * (1 - ((float) ghost3.moveProgress / GHOST_FRAMES))),
          (ghost3.position->y * SQUARE_SIZE) + (SQUARE_SIZE / 2) + Y_OFFSET - 
                      (Direction[ghost3.direction].y * SQUARE_SIZE * (1 - ((float) ghost3.moveProgress / GHOST_FRAMES)))};
